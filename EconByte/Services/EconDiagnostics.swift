...
final class EconDiagnostics: ObservableObject {
    ...
    init(transport: DiagnosticsTransporting,
        ...
        // ON by default: the crash handler installs at construction so it is in
        // place before the first line of app code that could crash. The two
        // guards below still apply — no DSN, or an SDK that cannot promise to
        // delete its own envelope cache, and nothing starts.
        setDiagnosticsConsent(true)
    }

    // MARK: Enablement

    /// Turns crash reporting on or off. There is no user-facing control for this
    /// in 1.1.2 — it is on — so the `false` path exists for tests and for a
    /// future kill switch, and `true` is what construction calls.
    ///
