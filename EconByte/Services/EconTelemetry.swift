...
enum TelemetrySchema {
    ...
    static let allowedProperties: [String: Set<String>] = [
        ...
        // RECONCILED (1.1.2): lineage A measured the other half of the same tap
        // — an UNLOCKED topic being opened. Without it `topic_locked_tapped_v1`
        // has no denominator. The topic itself never travels; `topic_id` is a
        // prohibited property.
        ...
        // Reconciled from lineage A (1.1): these five exist only because the
        // 1.1 growth stack has the subsystems that produce them — the
        // notification primer, the review-eligibility policy, the two consent
        // switches, and the ad adapter's dismissal callback. Lineage C had no
        // such subsystems, so its vocabulary could not name them.
        "notification_primer_viewed_v1": ["entry_point"],
        "review_prompt_eligible_v1": ["streak_bucket"],
    ...
    static let propertyKinds: [String: TelemetryPropertyKind] = [
        ...
        // The bundled 1.1 catalog (`curriculum-v1.1.json`, 15 topics / 120
        // cards) ships exactly three difficulty tiers. Lineage C declared two
        // because the v1.0 `cards.json` it was cut from only had two — the
        // reconciliation restores the catalog, so the vocabulary grows with it.
        // `testDeclaredDifficultiesMatchTheBundledCatalog` keeps the two in
        // step, so a new tier in the catalog fails here first.
        "difficulty": .enumerated(["intro", "intermediate", "advanced"]),
...
final class EconTelemetry: ObservableObject {
    ...
    enum Key {
        /// RECONCILED (1.1.2): analytics is **opt-in and off until the user
        /// turns it on**, which is what version 1.1 told users on the App Store
        /// ("Usage analytics and crash diagnostics are separate opt-in choices,
        /// both off by default") and what the 1.1 consent primer and its UI
        /// tests assume. Lineage C stored the *negative* (`ebAnalyticsOptOut`,
        /// absent == on); that would have silently reversed a published promise
        /// on update. The positive key below is lineage A's, so a 1.1 install
        /// that already opted in stays opted in across this update.
        static let consent = "econ.telemetry.consent"
    }
    ...
    init(transport: TelemetryTransporting,
        ...
        // OFF until the user opts in, and then only in a build that has
        // somewhere to send. Without a key nothing starts, however the user
        // answered before.
        if apiKey != nil, Self.isOptedIn(defaults) {
            startAnalytics()
        }
    }
    ...
    /// Whether the user has switched analytics on. Absent == never answered ==
    /// off.
    static func isOptedIn(_ defaults: UserDefaults) -> Bool {
        defaults.bool(forKey: Key.consent)
    }

    /// The Settings toggle, expressed as it reads to the user: `true` == share
    /// analytics. It starts `false`. Turning it off stops capture, clears this app's queue, and
    ...
    func setAnalyticsConsent(_ enabled: Bool) {
        defaults.set(enabled, forKey: Key.consent)
        if enabled {
