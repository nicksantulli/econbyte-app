...
// MARK: - Local diagnostic-code log (lineage A, EconByte 1.1)
//
// RECONCILED (1.1.2): this shipped in the approved 1.1 build 6 as
// `EconDiagnostics`. Lineage C introduced a *different* type of the same name —
// a real, crash-only Sentry facade. They do different jobs, so both survive
// under names that say which: `EconDiagnostics` is the Sentry crash reporter,
// and this is the bounded, scrubbed, **local-only** log of the app's own
// enumerated failure codes (ad load failed, notification schedule failed,
// catalog invalid). It has no DSN and no sink, exactly as it shipped, so it
// still transmits nothing. Routing these codes into Sentry as scrubbed messages
// is a deliberate follow-up, not something this reconciliation invented.
...
public struct EconDiagnosticLogOptions: Equatable {
...
public enum EconDiagnosticLogConfiguration {
    ...
    public static var options: EconDiagnosticLogOptions { EconDiagnosticLogOptions() }
}

public protocol EconDiagnosticLogSink: AnyObject {
    var isConfigured: Bool { get }
    func send(_ reports: [EconDiagnosticReport])
    func reset()
}

public final class EconNoOpDiagnosticLogSink: EconDiagnosticLogSink {
    public init() {}
...
public final class EconDiagnosticLog {
    ...
    private let sink: EconDiagnosticLogSink

    public init(defaults: UserDefaults = .standard,
                appVersion: String = "",
                buildNumber: String = "",
                osMajor: Int = 0,
                sink: EconDiagnosticLogSink = EconNoOpDiagnosticLogSink()) {
