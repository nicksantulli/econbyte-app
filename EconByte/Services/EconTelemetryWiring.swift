...
enum EBEntryPoint: String {
    ...
    /// RECONCILED (1.1.2): lineage A has a Session Complete screen — the place
...
/// RECONCILED (1.1.2): lineage C called this `EconGrowth`. Lineage A uses that
/// name for its growth **composition root** — the object that owns the
/// monetization, notification, review and consent subsystems and is injected
/// into every view. Two different things cannot share one name, so the
/// event-emission facade is `EBEvents`, matching the `EB…` prefix its own
/// vocabulary enums already use, and `EconGrowth` stays the composition root.
@MainActor
enum EBEvents {
    ...
    /// The unlocked half of the topic-grid tap. `accessState` is lineage A's
    /// `EconAccessState`, whose raw values are exactly the declared vocabulary.
    static func topicOpened(entryPoint: EBEntryPoint, accessState: EconAccessState) {
        EconTelemetry.shared.capture(TelemetryEvent("topic_opened_v1", [
            "entry_point": .string(entryPoint.rawValue),
    ...
    /// The launch count this app has recorded so far, for the bucketed
    /// `review_request_attempted_v1` property. Never transmitted raw.
    static func recordedLaunchCount(defaults: UserDefaults = .standard) -> Int {
    ...
    // MARK: Consent, notifications and review (reconciled from lineage A)
    //
    // These five exist only because the 1.1 growth stack has the subsystems
    // that produce them. Lineage C had no consent primer, no notification
    // policy and no review-eligibility policy, so its vocabulary could not
    // name them.

    static func notificationPrimerViewed(entryPoint: EBEntryPoint) {
        EconTelemetry.shared.capture(TelemetryEvent("notification_primer_viewed_v1", [
            "entry_point": .string(entryPoint.rawValue),
        ]))
    }

    static func reviewPromptEligible(streak: Int) {
        EconTelemetry.shared.capture(TelemetryEvent("review_prompt_eligible_v1", [
            "streak_bucket": .string(TelemetryBucket.streak(streak)),
        ]))
    }

    static func analyticsConsentChanged(enabled: Bool, entryPoint: EBEntryPoint) {
        EconTelemetry.shared.capture(TelemetryEvent("analytics_consent_changed_v1", [
            "enabled": .bool(enabled),
            "entry_point": .string(entryPoint.rawValue),
        ]))
    }

    static func diagnosticsConsentChanged(enabled: Bool, entryPoint: EBEntryPoint) {
    ...
    static func adDismissed(placement: EBAdPlacement, outcome: EBOutcome) {
        EconTelemetry.shared.capture(TelemetryEvent("ad_dismissed_v1", [
            "placement": .string(placement.rawValue),
            "outcome": .string(outcome.rawValue),
        ]))
    }
