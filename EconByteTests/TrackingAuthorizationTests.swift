...
///     tracking decision.
///  2. (1.1.3 build 16, App Review 2.1) The prompt is owed exactly while iOS
///     has no answer: an ask iOS never presented is asked again — on the next
///     launch too — and an answered status is never prompted.
...
final class TrackingAuthorizationTests: XCTestCase {
    ...
    /// The lifecycle hook never prompts and never requests before the decision.
    /// Build 16 does ask ATT at first launch, but only from
    ...
    /// foreground hook itself stays inert until the reader has answered.
    ...
    /// Build 16 (App Review 2.1, 2026-09-15): the state iOS leaves when it
    ...
    /// Build 16: a Remove Ads owner is asked too. App Review sandbox accounts can
    /// own Remove Ads from earlier reviews, and an unasked account is an "unable
    /// to locate the ATT prompt" rejection. They still never get an ad.
    func testRemoveAdsOwnersAreAskedButNeverGetAnAd() async {
        let log = CallLog()
        let adapter = LoggingAdapter(log: log)
        let tracking = StubTracking(status: .notDetermined, resolvesTo: .authorized, log: log)
        let monetization = makeMonetization(log, tracking: tracking, adapter: adapter)
        ...
        XCTAssertEqual(adapter.startCount, 0, "Remove Ads still means no ad SDK")
        XCTAssertFalse(monetization.canRequestAds)
    }

    /// Build 16: EEA/UK and unknown device regions are asked too (a review
    /// device's Region setting is invisible in the rejection), and DUD-224 still
