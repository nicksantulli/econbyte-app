...
| A12 | Banner layout: `.safeAreaInset(edge: .bottom)` on the tab scaffold / card VStack — sits above the tab bar and above the home indicator, reserves 0 pt until filled. **Verified by UI test with Google's test unit** (see report for fill result). | — | — | `testBannersSitAboveTheTabBarAndHomeIndicatorAndLeaveSearch` |
...
Ask to Buy's pending state is a `PurchaseManager` field exercised only in the skipped session test —
it needs the Xcode run or a sandbox Ask-to-Buy account to prove on a device.
